...
function getSheetsUrl() { return STATE.prefs?.sheetsUrl?.trim() ?? ''; }
...
async function loadFromSheets() {
  ...
  try {
    ...
    setSyncStatus('error');
    console.warn('[FitTrack] Sheets non raggiungibile, uso dati locali:', err.message);
    return false;
  }
}
...
async function _doSync(url) {
  ...
  try {
    ...
    setSyncStatus('error');
    console.warn('[FitTrack] Errore sync:', err.message);
  }
}
