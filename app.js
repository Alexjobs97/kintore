...
function renderCalendar() {
  ...
  if (!STATE.plan) {
    grid.innerHTML = '<p style="color:var(--muted);grid-column:1/-1;text-align:center;padding:40px">Carica prima una scheda nelle Impostazioni.</p>';
    return;
  }
  ...
  const month = (STATE.plan.month in MONTHS) ? MONTHS[STATE.plan.month] : now.getMonth();
  ...
  for (let day = 1; day <= lastDay.getDate(); day++) {
    ...
    const cell = document.createElement('div');
    cell.className = 'cal-day' + (isToday?' today':'') + (wo?' '+wo.type:'') +
...
function downloadICS() {
  ...
  const lines = ['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//筋トレ//IT','CALSCALE:GREGORIAN','X-WR-CALNAME:筋トレ'];
  ...
  for (let i = 0; i < 35; i++) {
    ...
    lines.push('BEGIN:VEVENT','UID:kintore-'+iso+'@app',
      'DTSTART;VALUE=DATE:'+dt,'DTEND;VALUE=DATE:'+dt,
      'SUMMARY:'+wo.emoji+' '+wo.label,
      'BEGIN:VALARM','TRIGGER:-PT30M','ACTION:DISPLAY','DESCRIPTION:Reminder: '+wo.label,'END:VALARM',
      'END:VEVENT');
  }
  lines.push('END:VCALENDAR');
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([lines.join('\r\n')], {type:'text/calendar'}));
  a.download = 'kintore.ics'; a.click();
